...
namespace Land_Pooling_Policy_Manager.Services
{
    public sealed class PolicyManagerService
    {
        ...
        /// <summary>
        /// Returns a list of "Name | Width m" strings used by the corner-type
        /// table editor. In the standalone policy manager there are no roads —
        /// we fall back to the canonical Baireni width set. When this service
        /// is hosted by the main RePlot app, a future overload can accept the
        /// project's actual road list via a provider delegate.
        /// </summary>
        public Task<List<string>> GetProjectRoadReferenceOptionsAsync(CancellationToken ct = default)
        {
            if (RoadReferenceOptionsProvider != null)
                return RoadReferenceOptionsProvider(ct);
        ...
        /// <summary>
        /// Optional hook: main RePlot host sets this so the corner-type editor
        /// can populate from the project's actual road inventory.
        /// </summary>
        public Func<CancellationToken, Task<List<string>>>? RoadReferenceOptionsProvider { get; set; }
        ...
        public async Task DeletePolicyAsync(int policySetId, CancellationToken ct = default)
        {
            ...
            // PolicyClause.ParentClauseId is a self-FK declared
            // ON DELETE RESTRICT in the .lpp schema, so a bulk DELETE of all
            // clauses in this policy fails with SQLite error 19 because parent
            // rows still have children referencing them at the moment of the
            // delete. Null out every ParentClauseId in this policy first — once
            // there are no incoming references, the bulk delete proceeds.
            await _context.PolicyClauses
                .Where(c => c.PolicySetId == policySetId && c.ParentClauseId != null)
                .ExecuteUpdateAsync(setters => setters.SetProperty(c => c.ParentClauseId, (int?)null), ct);
            await _context.PolicyClauses
