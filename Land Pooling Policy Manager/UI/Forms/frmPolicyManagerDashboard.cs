...
namespace Land_Pooling_Policy_Manager.UI.Forms
{
    ...
    public sealed partial class frmPolicyManagerDashboard : Form
    {
        ...
        private bool _isReloadingPolicyCombo;
        private int? _selectedParameterIdBeforeRefresh;

        public event EventHandler<string>? StatusChanged;

        // True when launched by the main app — used by host code (e.g. PolicyManagerLauncher)
        // to gate which child windows opened from the dashboard accept structural edits.
        ...
        /// <summary>
        /// Rebuilds the policy selector combo from the current <see cref="_policySummaries"/>
        /// list. When <paramref name="selectedPolicyId"/> is provided, the matching item is
        /// pre-selected. Sets a guard flag so the SelectedIndexChanged handler doesn't
        /// re-trigger a policy reload while we're populating.
        /// </summary>
        private void PopulatePolicyCombo(int? selectedPolicyId)
        {
            _isReloadingPolicyCombo = true;
            try
            {
                cboCurrentPolicy.Items.Clear();
                if (_policySummaries.Count == 0)
                {
                    cboCurrentPolicy.Items.Add("No policy");
                    cboCurrentPolicy.SelectedIndex = 0;
                    cboCurrentPolicy.Enabled = false;
                    return;
                }

                int targetIndex = 0;
                for (int i = 0; i < _policySummaries.Count; i++)
                {
                    PolicySet policy = _policySummaries[i];
                    cboCurrentPolicy.Items.Add(
                        $"{policy.PolicyName}  |  v{policy.VersionNo}  |  {policy.Status}");
                    if (selectedPolicyId.HasValue && policy.Id == selectedPolicyId.Value)
                        targetIndex = i;
                }

                cboCurrentPolicy.SelectedIndex = targetIndex;
                cboCurrentPolicy.Enabled = true;
            }
            finally
            {
                _isReloadingPolicyCombo = false;
            }
        }

        private async void cboCurrentPolicy_SelectedIndexChanged(object? sender, EventArgs e)
        {
            if (_isReloadingPolicyCombo)
                return;

            int idx = cboCurrentPolicy.SelectedIndex;
            if (idx < 0 || idx >= _policySummaries.Count)
                return;

            int chosenId = _policySummaries[idx].Id;
            if (_currentPolicy != null && _currentPolicy.Id == chosenId)
                return;

            await RunPolicyUiOperationAsync(
                "Switching policy...",
                () => LoadPolicyByIdAsync(chosenId, null, null, null));
        }

        private void ClearPolicyUi()
        {
            PopulatePolicyCombo(null);
            txtPolicyCode.Clear();
            txtPolicyName.Clear();
        ...
        private void LoadPolicyToUi(int? selectClauseId, int? selectSectionId)
        {
            ...
            PopulatePolicyCombo(_currentPolicy.Id);
        ...
        private async Task AddClauseAsync(int? parentClauseId)
        {
            ...
            PolicyClause clause = new()
            {
                ...
                ClauseCode = GenerateNextClauseCode(section, parentClauseId),
        ...
        /// <summary>
        /// Generates a section-prefixed clause code for a newly added clause.
        ///   • Top-level clause under section N → "N.X" where X is the next
        ///     integer not already used at the top of that section.
        ///   • Sub-clause whose parent code is "P" → "P.Y" where Y is the next
        ///     integer not already used directly beneath that parent.
        /// Falls back to leaving the code blank when the section code can't be
        /// resolved (e.g. ad-hoc section like "General").
        /// </summary>
        private string? GenerateNextClauseCode(string sectionHeading, int? parentClauseId)
        {
            if (_currentPolicy == null)
                return null;

            // For sub-clauses, branch off the parent's code: "3.4" → "3.4.1".
            if (parentClauseId.HasValue)
            {
                PolicyClause? parent = _currentPolicy.Clauses.FirstOrDefault(c => c.Id == parentClauseId.Value);
                if (parent == null || string.IsNullOrWhiteSpace(parent.ClauseCode))
                    return null;

                string parentCode = parent.ClauseCode!;
                string parentPrefix = parentCode + ".";
                HashSet<int> usedChild = _currentPolicy.Clauses
                    .Where(c => c.ParentClauseId == parentClauseId.Value)
                    .Select(c => ExtractLastSegment(c.ClauseCode, parentPrefix))
                    .Where(n => n.HasValue)
                    .Select(n => n!.Value)
                    .ToHashSet();
                return parentPrefix + NextFree(usedChild);
            }

            // Top-level under a section: look up the section's numeric code.
            PolicySectionDefinition? sectionDef = _currentPolicy.Sections
                .FirstOrDefault(s => string.Equals(s.Heading, sectionHeading, StringComparison.OrdinalIgnoreCase));
            if (sectionDef == null || string.IsNullOrWhiteSpace(sectionDef.SectionCode))
                return null;

            string sectionPrefix = sectionDef.SectionCode + ".";
            HashSet<int> usedTop = _currentPolicy.Clauses
                .Where(c => c.ParentClauseId == null &&
                            string.Equals(c.PolicySection, sectionHeading, StringComparison.OrdinalIgnoreCase))
                .Select(c => ExtractLastSegment(c.ClauseCode, sectionPrefix))
                .Where(n => n.HasValue)
                .Select(n => n!.Value)
                .ToHashSet();
            return sectionPrefix + NextFree(usedTop);
        }

        /// <summary>
        /// Returns the trailing integer of <paramref name="code"/> when it
        /// starts with <paramref name="expectedPrefix"/> and has no further
        /// dots. Returns null otherwise — so sub-clauses don't confuse the
        /// top-level numbering scan.
        /// </summary>
        private static int? ExtractLastSegment(string? code, string expectedPrefix)
        {
            if (string.IsNullOrEmpty(code) || !code.StartsWith(expectedPrefix, StringComparison.Ordinal))
                return null;

            string tail = code[expectedPrefix.Length..];
            if (tail.Length == 0 || tail.Contains('.'))
                return null;

            return int.TryParse(tail, out int value) ? value : null;
        }
        ...
        private void UpdateEditState()
        {
            ...
            // Structural edits (add/rename/delete/move sections, clauses,
            // parameters) are reserved for standalone mode. Hosted mode only
            // tunes values inside the dedicated Parameters / Lookup-tables
            // windows.
            ...
            btnApprove.Enabled = !_readOnlyMode && hasPolicy && canEditStructure;
            btnLockUnlock.Enabled = !_readOnlyMode && hasPolicy;
            btnExport.Enabled = hasPolicy;

            btnAddSection.Enabled = canEditStructure;
            btnRenameSection.Enabled = canEditStructure && hasSection;
            btnDeleteSection.Enabled = canEditStructure && hasSection;

            btnAddClause.Enabled = canEditStructure && hasSection;
            btnAddSubClause.Enabled = canEditStructure && hasClause;
            btnDuplicateClause.Enabled = canEditStructure && hasClause;
            btnDeleteClause.Enabled = canEditStructure && hasClause;
            btnMoveUp.Enabled = canEditStructure && hasClause;
            btnMoveDown.Enabled = canEditStructure && hasClause;
            btnOpenDiagram.Enabled = hasClause;
            btnAddParameter.Enabled = canEditStructure && hasClause;
            btnDeleteParameter.Enabled = canEditStructure && dgvParameters.SelectedRows.Count > 0;

            menuAddClause.Enabled = canEditStructure && hasSection;
            menuAddSubClause.Enabled = canEditStructure && hasClause;
            menuDuplicateClause.Enabled = canEditStructure && hasClause;
            menuDeleteClause.Enabled = canEditStructure && hasClause;
        }
