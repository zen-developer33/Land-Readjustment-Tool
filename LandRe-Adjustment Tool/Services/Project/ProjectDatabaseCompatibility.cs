...
namespace Land_Readjustment_Tool.Services.Project
{
    ...
    {
        private static readonly ConcurrentDictionary<string, DatabaseIdentity> EnsuredDatabases =
        ...
        {
            if (TryGetDatabaseIdentity(context, out DatabaseIdentity identity) &&
                EnsuredDatabases.TryGetValue(identity.Path, out DatabaseIdentity ensuredIdentity) &&
                ensuredIdentity.Equals(identity))
            {
                return;
            }
            ...
            if (TryGetDatabaseIdentity(context, out DatabaseIdentity updatedIdentity))
                EnsuredDatabases[updatedIdentity.Path] = updatedIdentity;
        }
        ...
        private static async Task EnsurePolicyTablesAsync(
            AppDbContext context,
            CancellationToken ct)
        {
            await context.Database.ExecuteSqlRawAsync(
            ...
            [
                ...
                "CREATE INDEX IF NOT EXISTS IX_tblPolicyParameters_PolicyClauseId ON tblPolicyParameters (PolicyClauseId);",
                ...
                "CREATE INDEX IF NOT EXISTS IX_tblPolicyLookupColumns_PolicyLookupTableId ON tblPolicyLookupColumns (PolicyLookupTableId);",
                "CREATE INDEX IF NOT EXISTS IX_tblPolicyLookupRows_PolicyLookupTableId ON tblPolicyLookupRows (PolicyLookupTableId);",
                "CREATE INDEX IF NOT EXISTS IX_tblPolicyLookupCells_PolicyLookupColumnId ON tblPolicyLookupCells (PolicyLookupColumnId);",
                ...
                "CREATE INDEX IF NOT EXISTS IX_tblPolicyAttachments_PolicyClauseId ON tblPolicyAttachments (PolicyClauseId);",
                "CREATE INDEX IF NOT EXISTS IX_tblPolicyAttachments_PolicySetId ON tblPolicyAttachments (PolicySetId);",
                "CREATE INDEX IF NOT EXISTS IX_tblPolicyAuditEntries_PolicySetId ON tblPolicyAuditEntries (PolicySetId);"
            ];

            foreach (string statement in indexStatements)
        ...
        private static bool TryGetDatabaseIdentity(
            AppDbContext context,
            out DatabaseIdentity identity)
        {
            identity = default;
